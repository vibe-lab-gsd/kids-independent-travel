...
hh_genders <- people %>%
  select(HOUSEID, PERSONID, R_SEX) |> ## Updated variable name to match 2009 variable
  pivot_wider(names_from = PERSONID,
...
relationships <- people |>
  mutate(adult = R_AGE > 17,
         mom = R_AGE > 17 & R_SEX == "02",
         dad = R_AGE > 17 & R_SEX != "02",
         ...
         driver_mom = R_AGE > 17 & DRIVER == "01" & R_SEX == "02",
         driver_dad = R_AGE > 17 & DRIVER == "01" & R_SEX != "02",
...
school_trips <- trips |>
  ...
  summarise(person_hh = first(person_hh),
            ...
            R_SEX = first(R_SEX),
            TRPMILES = sum(TRPMILES))|>   ##Removed density variables
  group_by(person_hh) |>
...
```{r, message = FALSE, warning = FALSE}
  ...
  # read data
  read_csv() |> 
  mutate(income_k = case_when(HHFAMINC == "01" ~ 2.5, 
                              HHFAMINC == "02" ~ 7.5,
                              HHFAMINC == "03" ~ 12.5,
                              HHFAMINC == "04" ~ 17.5,
                              HHFAMINC == "05" ~ 22.5,
...
|> ## Remove density variable
  ...
  mutate(log_income_k = log(income_k),
         log_distance = log(distance), ##Remove Density variable
         av_car = 1,
         av_walk = 1,
         av_bike = 1,
         alone_avail = 1,
