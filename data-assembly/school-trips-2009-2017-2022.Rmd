...
only (assuming this is a schooltrip).
...
trips17 <- here("nhts",
  ...
  read_csv(show_col_types = FALSE) |>
  mutate(year = 2017) |>
  select(-ONTD_P13, -ONTD_P12, -ONTD_P11) |> #filter out person >10 to match 2022 data
...
trips09 <- here("nhts",
  ...
  read_csv(show_col_types = FALSE) |>
  mutate(year = 2009) |>
  select(-ONTD_P15, -ONTD_P14, -ONTD_P13, -ONTD_P12, -ONTD_P11) |> #filter out person >10 to match 2022 data
  filter(WHYFROM == "01" & 
           WHYTO == "21" &
           TRPTRANS %in% c("01",
                           "02",
