...
# Estimate parameters
results = no_nests_biogeme_est.estimate()
...
pandas_results.to_csv('biogeme_parameters_no_nests.csv')

# Generate predictions
prob_adult_car = models.logit(V, av, 720)
prob_non_hh_car = models.logit(V, av, 730)

prob_alone_walk = models.logit(V, av, 810)
prob_adult_walk = models.logit(V, av, 820)
prob_non_hh_walk = models.logit(V, av, 830)

prob_alone_bike = models.logit(V, av, 910)
prob_adult_bike = models.logit(V, av, 920)
prob_non_hh_bike = models.logit(V, av, 930)
...
no_nests_biogeme_sim = bio.BIOGEME(database_sim, simulate)
no_nests_biogeme_sim.modelName = "predictions"

betaValues = results.getBetaValues ()
simulatedValues = no_nests_biogeme_sim.simulate(betaValues)

simulatedValues.to_csv('biogeme_preds_no-nests.csv')
