...
fn_make_dfidx <- function(my_trips, 
                          my_id, 
                          my_alts) {
  
  options <- unique(my_trips[[my_alts]])
  
  colnames(my_trips)[colnames(my_trips) == my_alts] ="alternatives"
  colnames(my_trips)[colnames(my_trips) == my_id] ="id"
